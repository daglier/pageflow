...
class UsuarioDAO
{
    ...
    public function restaurarBaseDatos($rutaArchivoSQL)
    {
        // Leer el contenido del archivo SQL
        $sql = file_get_contents($rutaArchivoSQL);
        if ($sql === false) {
            return false;
        }
        ...
        // Restaurar las verificaciones de claves foráneas
    ...
    {
        $stmt = $this->conexion->prepare("DELETE FROM " . TABLE_USUARIOS . " WHERE id_usuario=?");
        $stmt->bind_param("i", $id_usuario);
        return $stmt->execute();
    }
