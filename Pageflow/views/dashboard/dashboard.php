<?php
$nonce = base64_encode(random_bytes(16)); // Generar nonce aleatorio
header("Content-Security-Policy: default-src 'self'; script-src 'self' 'nonce-$nonce'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; font-src 'self';");
...
if (!isset($_SESSION["nombre_usuario"])) {
    ...
    header('location:../../'); //Envia al usuario al index.php
    exit();
}
    ...
    <main>
        ...
        <section id="configuracion" class="system-section configuracion-section">
            ...
            <div class="configuracion-form">
                ...
                <div class="row" id="configuracion-restauracion-div">
                    <label for="restauracion-respaldo"><i class="fa-solid fa-database"></i> Seleccionar respaldo:</label>
                    <select id="restauracion-respaldo-select" name="respaldo">
                        ...
                        foreach ($archivos as $archivo) {
                            if ($archivo !== "." && $archivo !== ".." && pathinfo($archivo, PATHINFO_EXTENSION) === "sql") {
                                $nombreAmigable = pathinfo($archivo, PATHINFO_FILENAME);

                                // Busca si hay una fecha en el formato yyyy-mm-dd
                                if (preg_match('/(\d{4})-(\d{2})-(\d{2})/', $nombreAmigable, $matches)) {
                                    $fechaFormateada = "{$matches[3]}/{$matches[2]}/{$matches[1]}"; // Formato dd/mm/yyyy
                                    $nombreAmigable = str_replace($matches[0], $fechaFormateada, $nombreAmigable);
                                }

                                // Reemplazar guiones bajos por espacios, pero dejar las fechas con guiones
                                $nombreAmigable = preg_replace('/_+/', ' ', $nombreAmigable);
                                $nombreAmigable = ucwords($nombreAmigable);

                                echo "<option value='$archivo'>$nombreAmigable</option>";
                            }
                        }
                        ?>
                    </select>

                    <div class="row button" id="configuracion-restauracion-btn">
                        <button id="restauracion-btn">
                            <i class="fa-solid fa-database"></i> Restaurar base de datos
                        </button>
                    </div>
                </div>
            </div>
